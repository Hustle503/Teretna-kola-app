...
# 🔹 Funkcija za čuvanje poslednjeg stanja u DuckDB
def save_last_state(df: pd.DataFrame):
    con = duckdb.connect(DB_FILE)
    con.execute("CREATE TABLE IF NOT EXISTS poslednje_stanje AS SELECT * FROM df LIMIT 0")
    con.execute("DELETE FROM poslednje_stanje")  # obriši staro stanje
    con.execute("INSERT INTO poslednje_stanje SELECT * FROM df")
    con.close()

# 🔹 Funkcija za učitavanje poslednjeg stanja iz DuckDB
def load_last_state():
    con = duckdb.connect(DB_FILE)
    try:
        df = con.execute("SELECT * FROM poslednje_stanje").fetchdf()
    except Exception:
        df = pd.DataFrame()
    con.close()
    return df
...
def get_duckdb_connection():
    """Jedna globalna konekcija po Streamlit sesiji."""
...
if selected_tab == "📌 Poslednje stanje kola":
    ...
    # 🔹 Učitaj iz baze ako već postoji
    if "df_last" not in st.session_state:
        st.session_state.df_last = load_last_state()

    if st.button("🔎 Prikaži poslednje stanje kola", key="btn_last_state"):
        if st.session_state.df_last is None or st.session_state.df_last.empty:
            try:
                q_last = """
                SELECT 
                ...
                LEFT JOIN (
                    SELECT 
                        TRY_CAST("Broj vagona" AS BIGINT),
                        kola.*
                    FROM kola
                ) k
                  ON TRY_CAST(s."Broj kola" AS BIGINT) = k."Broj vagona"
                ...
                st.success("✅ Poslednje stanje je uspešno izračunato i sačuvano.")
            except Exception as e:
                st.error(f"Greška u upitu: {e}")

    # Ako imamo podatke, prikaži ih
    if st.session_state.df_last is not None and not st.session_state.df_last.empty:
        df_last = st.session_state.df_last        
...
if selected_tab == "🛑 Provera grešaka po statusu":
    ...
    con = duckdb.connect("BASE_DIR + '/'kola_sk.db")  # prilagodi putanju
...
if selected_tab == "🚂 Kretanje vozova":
    ...
    # 🔹 Konekcija
    con = duckdb.connect("BASE_DIR + '/'kola_sk.db")
